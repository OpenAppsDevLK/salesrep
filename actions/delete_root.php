...
// if search keyword is seet on URL....
 if(isset($_GET["SearchKey"])){
     ...
     $rowCount = count($_POST["check_list"]);
            ...
            $stmt = $db->prepare("DELETE FROM cp_root WHERE id='" . $_POST["check_list"][$i] . "'");
            $stmt->execute(); 
            $stmt->close();
            //$stmt2->close();
...
header('Location: ../index.php?page=Roots&PageNo=1'); 
...
//Jump to the same page after deleteing the image
header('Location: ../index.php?page=Roots&PageNo='. $PageNo); 
